...
const { entries, isLoading, isEmpty, error, load, refresh, hasLoaded, isGuestMode } = useLeaderboard(keyRef)
const loadTimeout = ref<ReturnType<typeof setTimeout> | null>(null)
const isCompact = computed(() => !!props.compact)

function handleGuestScoreAdded() {
...
onMounted(() => {
  window.addEventListener('guest-score-added', handleGuestScoreAdded)
})

onBeforeUnmount(() => {
  window.removeEventListener('guest-score-added', handleGuestScoreAdded)
})
...
<template>
  <div class="leaderboard-card" :class="{ 'compact-shrink': isCompact }">
    <div class="leaderboard-title">
      Leaderboard
      <span v-if="isGuestMode" class="guest-mode-badge">Local Scores</span>
    </div>
    ...
    <div class="leaderboard-notice">
      <div v-if="isGuestMode" class="update-notice guest-notice">
        <span class="guest-icon">📱</span>
        Local score history (Guest mode)
      </div>
      <div v-else class="update-notice">Leaderboard updates every 15 minutes</div>
    </div>
  </div>
</template>
...
.guest-notice { color: var(--neon-purple); font-weight: 600; display: flex; align-items: center; gap: 6px; }
.guest-icon { font-size: 1rem; }
.guest-mode-badge {
