...
// Reactive ref for guest avatar to ensure reactivity
const guestAvatar = ref<string | null>(localStorage.getItem(GUEST_AVATAR_KEY))

const userAvatar = computed(() => {
  if (isGuestMode.value) {
    if (guestAvatar.value) return guestAvatar.value
    return RoboDoughAvatar
  }
    ...
    try {
      ...
      if (userId) {
        if (userId.startsWith('guest-')) {
          ...
          tokens.value = 0
          tickets.value = 0
          const storedGuestAvatar = localStorage.getItem(GUEST_AVATAR_KEY)
          guestAvatar.value = storedGuestAvatar
          
          // Sync guest avatar to TurboWarp namespace if set
          if (storedGuestAvatar) {
            // Try to get base64 from localStorage first, otherwise convert
            const existingBase64 = localStorage.getItem('arcadeUserAvatarBase64') || ''
            if (existingBase64 && existingBase64.startsWith('data:')) {
              writeNamespaceFields({ 
                avatarURL: storedGuestAvatar,
                avatar64: existingBase64
              })
            } else {
              // Convert to base64
              fetch(storedGuestAvatar)
                ...
                .catch(() => {
                  ...
                  writeNamespaceFields({ avatarURL: storedGuestAvatar })
                })
            }
          }
        } else {
...
function handleStorage(e: StorageEvent) {
  ...
  if (e.key === GUEST_AVATAR_KEY) {
    guestAvatar.value = e.newValue
  }
}

function handleGuestAvatarUpdate(e: CustomEvent) {
  const avatarPath = (e.detail as any)?.avatarPath
  if (avatarPath) {
    guestAvatar.value = avatarPath
  }
}
...
onMounted(() => {
  ...
  // Initialize guest avatar from storage
  const storedGuestAvatar = localStorage.getItem(GUEST_AVATAR_KEY)
  guestAvatar.value = storedGuestAvatar
  
  // Sync guest avatar to TurboWarp namespace if set and user is in guest mode
  if (storedGuestAvatar && isGuestMode.value) {
    // Try to get base64 from localStorage first, otherwise convert
    const existingBase64 = localStorage.getItem('arcadeUserAvatarBase64') || ''
    if (existingBase64 && existingBase64.startsWith('data:')) {
      writeNamespaceFields({ 
...
function handleLogout() {
  // Clear guest avatar selection
  localStorage.removeItem(GUEST_AVATAR_KEY)
  guestAvatar.value = null
  
  // Clear avatar from TurboWarp namespace
  try {
    const obj = parseNamespaceValue(localStorage.getItem(NAMESPACE_KEY))
    if (obj.data) {
      delete obj.data.avatarURL
      delete obj.data.avatar64
      obj.time = Math.floor(Date.now() / 1000)
      localStorage.setItem(NAMESPACE_KEY, JSON.stringify(obj))
    }
  } catch {
