...
import { addGuestScore } from '@/utils/storage'
...
async function handleTokenStatusCheck() {
  ...
  try {
    ...
    if (!userId) {
      ...
      if (!pb) {
        unlimitedPlay.value = true
        info('Guest mode: no backend configuration. Play allowed without prize tickets.', { timeout: 3000, position: 'top-center' })
        return
      }
      try {
  ...
  if (wasIngame && !isIngame.value) {
    ...
    if (Number.isFinite(best) && best > 0) {
      if (isGuestMode.value) {
        const gameKey = manifest.value?.gameName || base
        addGuestScore(gameKey, best)
        window.dispatchEvent(new CustomEvent('guest-score-added', { detail: { gameKey, score: best } }))
      } else {
